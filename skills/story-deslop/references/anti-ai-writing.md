...
| 动作句流水账（全文级） | “句句都是谁做了什么”：人物主语 + 动作短句挤满叙述主干，心理、叙述、环境句被挤光。步进式零信息动作（站起来/拿起来/掏出来/划开）直接删或合并；能用叙述直接讲的别演（“他这三年过得不好”优于“他叹了口气，垂下眼，把碗放回桌上”）；动作只留能改变判断、关系或下一动作的；对白不句句配动作标签 |
...
`check-ai-patterns.js` 的 `action-list-tic`（单段动作动词堆叠）与 `action-sentence-parade`（全文动作句密度：人物主语短句 + 强动作动词，阈值 6 句/50%/12 叙述句起报）只提示，不是 blocking。功能性打斗/追逐/仪式步骤若动作链本身承担信息，可保留或标 `[需复核]`。番茄高分样本中该类命中为 0，因此适合作为“需通读”的风格提示，而不是硬性失败项。

**零信息动作删除测试**：对每个动作句做一次删除测试——“删掉这句，判断、关系、空间、风险或下一动作会不会变？”不会变的句子（放下碗/站起来/划开屏幕/合上笔帽/把手机揣回兜里）就是监控步骤，删或并进相邻句；会变的（拉开抽屉看见信/把水倒进绿萝）才单独保留。删除后若整段变薄，优先恢复叙述或心理直写，不靠加动作补厚。
