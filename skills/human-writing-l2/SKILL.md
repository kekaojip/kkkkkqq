...
以下六条由"完成度波动"统一解释，保持 v0.1 原义不变，而不是被替换。
...
### 2.6 动作密度：叙述是主干，动作是点缀

模型为了"表现"会把情绪、关系、信息全部物化成可见动作，正文变成"句句都是谁做了什么"的监控流水账。真人正文大部分句子在做别的事：直接讲、直接想、交代环境物件、对白自己站着。动作句是点缀不是主干；零信息动作（删掉后判断、关系、空间、风险、下一动作都不变）直接删或并进相邻句，只保留能改变一件事的动作。不句句配动作标签，不把叙述句、心理直写当偷懒。详见 `references/l2-core.md` §9。
