...
J13 SIMULATOR_ENERGY: 模拟器文情绪与系统浓度（母本对齐）：
    感叹号 ≥15/章（情绪节点必须喊出来，禁止冷静腔/压住狂喜）；
    【】面板行 ≥25/章；≥2 个系统互动场景（抽取/选择/光幕/结算/吐槽/指引）；
    FAIL → 返回 S2 剧情块补 SYSTEM 镜头 + 情绪外放（不在成品上补标点）
J14 OPEN_END_DIVERSITY: 开结尾多样性（防模板化，契约见 simulator-novel-production-contract §4.1/4.2）：
    开篇非纯氛围（同 J7）；结尾不得默认以系统面板/【可模拟人生】/"来！"收尾；
    连续两章开头手法/结尾手法不得同型（对照上一章正文核验）；
    无钩子自然断章合法，不得因此判 FAIL；
    FAIL → 返回 S2 剧情块改开头/结尾拍型后重跑（不在成品上贴钩子或删钩子）
```
