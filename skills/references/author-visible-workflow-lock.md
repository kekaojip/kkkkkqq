...
禁止把 Fire、Canon、Tracking、Chapter Complete 重新加成独立进度行。

## 9. 防膨胀锁

```text
AUTHOR_VISIBLE_STEP_COUNT_MAY_NOT_INCREASE_AUTOMATICALLY
...
> 第一章成功体验就是唯一前台真源：书籍基础 → 母本拆解 → 剧情块 → 人物块 → 必要情绪线 → 正文。章节情绪线只在需要时出现，并固定为“章初状态 / 情绪推进 / 章末状态 / 带到下一章”的极薄模板。Fire、Fidelity、Combination 都藏在剧情块内部，Canon、Tracking、本章完成都藏在正文采用后的后台闭环。以后修 bug 只能加内部安全网，不能再给作者长新步骤。
